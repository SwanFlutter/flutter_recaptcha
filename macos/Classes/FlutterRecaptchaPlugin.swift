...
public class FlutterRecaptchaPlugin: NSObject, FlutterPlugin {
  ...
  private func verify(arguments: [String: Any]?, result: @escaping FlutterResult) {
    ...
    let resultMap: [String: Any] = [
      "success": success,
      "token": success ? "macos_token_\(Int(Date().timeIntervalSince1970 * 1000))" : NSNull(),
      "score": score,
  ...
  private func isBiometricAvailable() -> Bool {
    ...
    let canEvaluate = context.canEvaluatePolicy(.biometryAny, error: &error)

    // Also check if there's actual biometric hardware
    if canEvaluate {
